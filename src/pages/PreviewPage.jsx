...
export default function PreviewPage() {
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-xl mx-auto px-4 py-20 text-center">
          ...
          <p className="text-gray-500 mb-6">
            Este currículo pode ter sido excluído ou o link está incorreto.
          </p>
          <Link to="/historico" className="text-indigo-600 hover:underline font-medium">
            Ver histórico →
          </Link>
        </main>
      </div>
  ...
  async function handleExportar() {
    setExportando(true)
    try {
      const nomeArquivo = `curriculo-${vaga.titulo
        .toLowerCase()
        .replace(/\s+/g, '-')
        .slice(0, 40)}`
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/*
        Layout: sidebar fixa à esquerda + área do currículo à direita.
        max-w-screen-xl dá mais folga que max-w-5xl (1024px → 1280px).
        O currículo dentro de sua coluna vai escalar automaticamente
        via o ResizeObserver do CurriculoPreview.
      */}
      <div className="max-w-screen-xl mx-auto px-4 py-8 flex flex-col lg:flex-row gap-6 items-start">

        {/* ── Painel lateral ── */}
        <aside className="w-full lg:w-64 xl:w-72 flex-shrink-0 space-y-4 lg:sticky lg:top-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div
              className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full border ${corBg[cor]}`}
            >
              {texto}
            </div>
          ...
          {/* Skills que faltam */}
          {vaga.keywords?.filter(
            k => !vaga.curriculoGerado.skillsPrioritarias?.some(
              s => s.nome.toLowerCase() === k
            )
          ).length > 0 && (
            <div className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        {/* ── Área do currículo ── */}
        {/*
          min-w-0 é essencial em itens flex: evita que o filho ignore o
          flex-shrink e transborde. O CurriculoPreview usa ResizeObserver
          para ler a largura real deste container e escalar o A4 (794px)
          proporcionalmente, sem scroll horizontal.
        */}
        <div className="flex-1 min-w-0">
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
            ...
            <div className="bg-gray-50 border-b border-gray-200 px-4 py-2.5 flex items-center gap-2">
              ...
              <p className="text-xs text-gray-500 ml-2 truncate">{vaga.titulo}</p>
            </div>

            {/* O currículo escala para caber aqui sem overflow */}
